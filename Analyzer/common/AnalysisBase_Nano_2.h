...
*/
    ...
    if (nLepSelect==1) {
      if (nEleSelect==1) {
	MT = sqrt( 2*data.ele.pt[iEleSelect[0]]*data.rawmet.pt[0] * (1 - std::cos(data.rawmet.phi[0]-data.ele.phi[iEleSelect[0]])) );
      } else if (nMuSelect==1) {
	MT = sqrt( 2*data.mu.pt[iMuSelect[0]]*data.rawmet.pt[0] * (1 - std::cos(data.rawmet.phi[0]-data.mu.phi[iMuSelect[0]])) );
      }
    }
    MT_vetolep = 9999;
    if (nLepVeto==1) {
      if (nEleVeto==1) {
	MT_vetolep = sqrt( 2*data.ele.pt[iEleVeto[0]]*data.rawmet.pt[0] * (1 - std::cos(data.rawmet.phi[0]-data.ele.phi[iEleVeto[0]])) );
      } else if (nMuVeto==1) {
	MT_vetolep = sqrt( 2*data.mu.pt[iMuVeto[0]]*data.rawmet.pt[0] * (1 - std::cos(data.rawmet.phi[0]-data.mu.phi[iMuVeto[0]])) );
      }
    }
...
//    ChargedHadronIsoEACorr.push_back(data.pho.ChargedHadronIsoEAcorrectedsp15[i]);
  ...
  met_1l. SetPtEtaPhi(data.rawmet.pt[0], 0, data.rawmet.phi[0]);
  met_1vl.SetPtEtaPhi(data.rawmet.pt[0], 0, data.rawmet.phi[0]);
  met_ll. SetPtEtaPhi(data.rawmet.pt[0], 0, data.rawmet.phi[0]);
  if (nLepSelect==1) {
    TVector3 lep_met;
  ...
  TVector3 met_pho;
  met_pho.SetPtEtaPhi(data.rawmet.pt[0], 0, data.rawmet.phi[0]);
  if (nPhotonSelect==1) {
    TVector3 pho_met;
  ...
  met_fakepho.SetPtEtaPhi(data.rawmet.pt[0], 0, data.rawmet.phi[0]);
  if (nPhotonFake==1) {
    TVector3 pho_met;
...
*/
  ...
  if (hemis_AK4.size()==2) {
    ...
    shifted_met.SetPtEtaPhi(data.rawmet.pt[0], 0, data.rawmet.phi[0]); // shifted_met is complicated one for us because of the lack of exact met variable on new Ntuple
