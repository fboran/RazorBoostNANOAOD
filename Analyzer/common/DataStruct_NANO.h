...
public:
  ...
  class EventData {
    ...
    public:
      ...
      unsigned int size;
      std::vector<double> MetUnclustEnUpDeltaX;
      std::vector<double> MetUnclustEnUpDeltaY;
      std::vector<double> covXX;
      std::vector<double> covXY;
      std::vector<double> covYY;
      std::vector<double> phi;
      std::vector<double> pt;
      std::vector<double> significance;
      std::vector<double> sumEt;
      std::vector<double> fiducialGenPhi;
      std::vector<double> fiducialGenPt;

      void init() {
        size=9999;
        init_vec(MetUnclustEnUpDeltaX);
        init_vec(MetUnclustEnUpDeltaY);
        init_vec(covXX);
        init_vec(covXY);
        init_vec(covYY);
        init_vec(phi);
        init_vec(pt);
        init_vec(significance);
        init_vec(sumEt);
        init_vec(fiducialGenPhi);
        init_vec(fiducialGenPt);
    }
    ...
    public:
      ...
      int TriplePhoton_20_20_20_CaloIdLV2;
      int TriplePhoton_20_20_20_CaloIdLV2_R9IdVL;
      int TriplePhoton_30_30_10_CaloIdLV2;
      int TriplePhoton_30_30_10_CaloIdLV2_R9IdVL;
      int TriplePhoton_35_35_5_CaloIdLV2_R9IdVL;
      int Photon25;
      int Photon33;
      int Photon50;
      int Photon75;
      ...
      int Dimuon0_Upsilon_Muon_L1_TM0;
      int Dimuon0_Upsilon_Muon_NoL1Mass;
    ...
    public:
      ...
      std::vector<double> btagCSVV2;
      std::vector<double> btagDeepB;
      std::vector<double> btagDeepC;
      std::vector<double> btagDeepFlavB;
      std::vector<double> chEmEF;
      std::vector<double> chHEF;
      std::vector<double> eta;
      std::vector<double> mass;
      std::vector<double> neEmEF;
      std::vector<double> neHEF;
      std::vector<double> phi;
      std::vector<double> pt;
      std::vector<double> qgl;
      std::vector<double> rawFactor;
      std::vector<double> bReg;
      std::vector<double> electronIdx1;
      std::vector<double> electronIdx2;
      std::vector<double> jetId;
      std::vector<double> muonIdx1;
      std::vector<double> muonIdx2;
      std::vector<double> nConstituents;
      std::vector<double> nElectrons;
      std::vector<double> nMuons;
      std::vector<double> puId;
      std::vector<double> genJetIdx;
      std::vector<double> hadronFlavour;
      std::vector<double> partonFlavour;
      std::vector<double> cleanmask;
      std::vector<double> nJet;
    ...
    unsigned int size;
      ...
      //size==9999;
        ...
        init_vec(rawFactor);
        init_vec(bReg);
        init_vec(electronIdx1);
        init_vec(electronIdx2);
        init_vec(jetId);
        init_vec(muonIdx1);
        init_vec(muonIdx2);
        init_vec(nConstituents);
        init_vec(nElectrons);
        init_vec(nMuons);
        init_vec(puId);
        init_vec(genJetIdx);
